/**
 * This script purges all existing prompts from the database
 * and then reimports them from our CSV file.
 * 
 * WARNING: This will delete all existing prompts!
 * 
 * Usage:
 * 1. Edit the new-prompts.csv file with your prompts
 * 2. Run: node purge-prompts.js
 */
...
// Function to purge all prompts from the database
async function purgeAllPrompts() {
  try {
    console.log("Purging all existing prompts from the database...");
    
    const response = await fetch('http://localhost:5000/api/admin/purge-prompts', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer strangers_admin' // Using the default admin key
      },
    });
    ...
    console.log(`Successfully purged ${result.count} prompts from the database.`);
    return true;
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    if (prompts.length > 0) {
      ...
      // First purge all existing prompts
      const purgeSuccess = await purgeAllPrompts();
      
      if (purgeSuccess) {
        // Then import the new prompts
        await importPrompts(prompts);
      } else {
        console.error("Failed to purge existing prompts. Import aborted.");
      }
    }
  } catch (error) {
