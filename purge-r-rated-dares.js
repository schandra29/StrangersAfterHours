...
// If the above import fails, we may need to use TypeScript instead
import { eq } from 'drizzle-orm';
import { challenges } from './shared/schema.js';
import fs from 'fs';
...
async function purgeRRatedDares() {
  try {
    ...
    const countBefore = Number(countBeforeResult[0].count);
    console.log(`Found ${countBefore} existing R-Rated dares`);
    ...
    // Verify deletion
    const countAfterResult = await db
      .select({ count: db.fn.count() })
      .from(challenges)
      .where(eq(challenges.type, 'R-Rated Dare'));
      
    const countAfter = Number(countAfterResult[0].count);
    console.log(`Remaining R-Rated dares: ${countAfter}`);
...
async function main() {
  try {
    ...
    console.log(`Final count of R-Rated dares in database: ${count}`);
